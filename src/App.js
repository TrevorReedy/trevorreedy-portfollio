...
export const PROJECTS = [
  ...
  },
  {
    ...
    description:
      "A in house tool for CPR to use to calculate labor prices and to give reliable, fast, knowledge based quotes",
...
function Home() {
  ...
  const [scrollPhysicsEnabled, setScrollPhysicsEnabled] = useState(true);
  ...
  useEffect(() => {
    ...
    const iconElements = mobile
      ? buildRandomRows(container, ICONS, iconsPerRow, iconSize)
      : buildEvenIconRows(container, ICONS, iconsPerRow, iconSize);
    ...
    positionedIcons.forEach(
      ({ iconName, img: icon, pos, visualWidth, visualHeight }, index) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (!scrollPhysicsEnabled) return;
      ...
      bodies.forEach(({ body }, index) => {
        ...
        Body.setAngularVelocity(
          body,
          Math.max(-cap, Math.min(cap, body.angularVelocity + added))
        );
      });
...
<div className="physics-toggle-wrapper">
  <img
    className="physics-toggle-icon"
    width="50"
    height="50"
    src={scrollPhysicsEnabled ? "/toggleStates/gravity.svg" : "/toggleStates/zeroGravity.svg"}
    alt={scrollPhysicsEnabled ? "Scroll physics enabled" : "Scroll physics disabled"}
  />

  <label className="switch" title="Toggle scroll physics">
    <input
      type="checkbox"
      checked={scrollPhysicsEnabled}
      onChange={() => setScrollPhysicsEnabled((enabled) => !enabled)}
    />
          ...
          <div className="contact-content">
            <div className="contact-info">
              <p>
                ...
                interesting problems. Tell me what you're working on and how I can help.
              </p>
